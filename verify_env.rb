...
RSpec.describe do
  ...
  it 'Atom or Sublime Text is installed.' do
    atom_search = lsregister_grep('Atom')
    subl_search = lsregister_grep('Sublime Text')
    expect([atom_search, subl_search]).to_not satisfy { |searches| searches.all?(&:empty?) },
  ...
  it 'XCode is installed.' do
    xcode_search = lsregister_grep('xcode')
    expect(xcode_search).to match(%r{/Applications/Xcode}),
                            "XCode is not installed. Open the Mac App Store and install 'Xcode'."
  end
  ...
  it 'Version 8.2 or greater of Xcode Command Line Tools is installed.' do
    xcode_cli = `pkgutil --pkg-info=com.apple.pkg.CLTools_Executables`
    expect(xcode_cli).to match(/version: 8\.[0-9]+/),
      "Xcode Command Line Tools 8.2 or greater are not installed. Open the Mac App Store and install and/or update 'Xcode'."
  end
